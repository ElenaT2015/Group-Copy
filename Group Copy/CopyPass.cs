...
namespace Group_Copy
{
    [TransactionAttribute(TransactionMode.Manual)]
    public class CopyPass : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                XYZ point = uidoc.Selection.PickPoint("Выберете точку вставки");

                Room roomselected = GetRoomByPoint(doc, point);
                XYZ roomCenternew = GetElementCenter(roomselected);

                Transaction transaction = new Transaction(doc);
                transaction.Start("Копирование группы объектов");
                doc.Create.PlaceGroup(roomCenternew + offset, group.GroupType);
                transaction.Commit();
            }
